...
// Legado 书源规则引擎 —— 移植自 MoonTVPlus src/lib/legado.client.ts。
//
// 原项目跑在 Next.js 服务端(Node),本项目跑在 Tauri WebView(浏览器)。差异适配:
//   - `crypto`(createHash/createCipheriv) → legado-crypto.ts(crypto-js:md5/sha1/aes-cbc)
//   - `Buffer`                             → legado-crypto.ts 的 BufferShim(Uint8Array 子类)
//   - Node `vm`(runInNewContext 沙箱)      → new Function 沙箱(见 runInSandbox)
//   - `@xmldom/xmldom` + `xpath`           → WebView 原生 DOMParser + document.evaluate
//   - 服务端 `fetch`(无 CORS)              → readingFetchText(走 Rust script_http_bytes,GBK 解码)
//   - `validateProxyUrlServerSide`(SSRF)   → 本地应用无服务端可保护,降级为 isHttpUrl
//   - `getConfig()`(admin+env 书源)        → config.ts 的 resolveLegadoSources(localStorage)
//   - `/api/books/image` 图片代理           → dyproxy wrapImage(带 UA/Referer)
//
// 导出名对齐 DouyTV 侧已有调用:`legadoClient` 实例 + `normalizeImportedLegadoSources`。
import * as cheerio from 'cheerio';
import he from 'he';

import { resolveLegadoSources } from './config';
import {
  BufferShim,
  md5Hex,
  sha1Hex,
...
/** Node Buffer 的浏览器替身 —— 引擎内部与书源 <js> 规则都可能引用 `Buffer`。 */
...
const DEFAULT_LEGADO_SEARCH_PAGES = 5;
const LEGADO_CACHE_TTL_MS = 10 * 60 * 1000;
...
/** base64url 编解码(WebView btoa/atob 只做标准 base64,需手动转 URL-safe)。 */
function toBase64Url(input: string): string {
  return bytesToStr(strToBytes(input, 'utf8'), 'base64')
    .replace(/\+/g, '-')
    .replace(/\//g, '_')
    .replace(/=+$/, '');
}
...
function fromBase64Url(input: string): string {
  return bytesToStr(strToBytes(input, 'base64'), 'utf8');
}
...
function encryptTongrenKeyword(plainText: string) {
  ...
  const cipherBytes = aesCbcEncrypt(strToBytes(String(plainText), 'utf8'), key, iv);
  return encodeURIComponent(bytesToStr(cipherBytes, 'base64'));
}

function decryptTongrenOpenUrl(encryptedBase64: string, num: string, source?: LegadoBookSourceRule) {
  try {
    const numStr = bytesToStr(strToBytes(String(num || ''), 'base64'), 'utf8');
    const userAgent = (asObjectHeader(source?.header)['User-Agent'] || 'Mozilla/5.0 (Linux; Android 9) Mobile Safari/537.36').toLowerCase();
    // key 是 md5 十六进制字符串(32 字符)的 UTF-8 字节 = 32 字节,正好 AES-256。
    const key = strToBytes(md5Hex(userAgent + numStr), 'utf8');
    const encryptedData = strToBytes(String(encryptedBase64 || ''), 'base64');
    const iv = encryptedData.subarray(0, 16);
    const data = encryptedData.subarray(16);
    return bytesToStr(aesCbcDecrypt(data, key, iv), 'utf8');
  } catch {
    return '';
  }
}
...
function runJsSnippetRaw(code: string, context: Record<string, any>, _timeout = 1000): any {
  const sourceRuntime = makeSourceRuntime(context.source, context.baseUrl);
  let lastPut: any = null;
  const java = {
    base64Encode: (value: unknown) => Buffer.from(String(value ?? ''), 'utf8').toString('base64'),
    base64Decode: (value: unknown) => Buffer.from(String(value ?? ''), 'base64').toString('utf8'),
    md5Encode: (value: unknown) => md5Hex(String(value ?? '')),
  ...
  // Node `vm.runInNewContext(sandbox, { timeout })` → `new Function`。
  // WebView 无 vm,用 new Function 把 sandbox 键当形参注入(同步执行,无法强制超时;
  // 书源 JS 都是纯计算,无 while(true) 风险,超时保护在这里让位)。三段式回退与原项目一致:
  //   1) 函数体直接执行(书源里常见 `result = ...` 赋值 / 末尾 return)
  //   2) 函数体后补 `if (res) return res`(书源用裸变量 res 承载结果)
  //   3) 当作单表达式 `return (body)`(如 @js:JSON.stringify({...}))
  const sandboxKeys = Object.keys(sandbox);
  const sandboxValues = sandboxKeys.map((key) => sandbox[key]);
  const runVariant = (source: string): any => {
    // eslint-disable-next-line no-new-func
    const fn = new Function(...sandboxKeys, source);
    return fn(...sandboxValues);
  };
  const pickResult = (result: any): any => {
  ...
  for (const variant of variants) {
    try {
      const picked = pickResult(runVariant(variant));
...
function readJsonRule(json: any, rule?: string, source?: BookSource, baseUrl?: string): string {
  ...
  if (isJsRuleString(trimmed)) {
    ...
    if (/java\.md5Encode/.test(trimmed)) return md5Hex(resultText);
...
function selectXPath($: cheerio.CheerioAPI, root: cheerio.Cheerio<any>, rule: string): { nodes: cheerio.Cheerio<any>; attr: string; value?: string } {
  ...
  // Node `@xmldom/xmldom` + `xpath` → WebView 原生 DOMParser + document.evaluate。
  // 浏览器 XPath 只吃 XML/HTML Document,不认 cheerio 节点,所以把当前子树序列化成
  // HTML 再 parse。document.evaluate 用 ORDERED_NODE_SNAPSHOT_TYPE 兼容节点集与属性/文本节点。
  try {
    ...
    for (let i = 0; i < snapshot.snapshotLength; i += 1) {
      ...
      if (!node) continue;
      // 2=Attr 3=Text 4=CDATA → nodeValue;元素 → textContent
      if (node.nodeType === 2 || node.nodeType === 3 || node.nodeType === 4) {
        if (node.nodeValue) values.push(node.nodeValue);
      } else if (node.textContent) {
        values.push(node.textContent);
      }
    }
    if (values.length > 0) return { nodes: root, attr: '', value: values.filter(Boolean).join('\n') };
  } catch {
    // document.evaluate 抛错(表达式非法 / 返回非节点集)时回退到下面的 CSS 近似转换
  }
...
function applyContentJsRule(value: string, jsRule: string): string {
  ...
  if (encryptedParams) {
    try {
      ...
      // Node `crypto.createDecipheriv('aes-128-cbc', key, iv)` → aesCbcDecrypt(crypto-js)。
      // iv 是密文前 16 字节,key 固定,padding PKCS7。
      const decodedBytes = aesCbcDecrypt(
        encrypted.subarray(16),
        strToBytes('5V&RoR%Jf@pJPydF', 'utf8'),
        encrypted.subarray(0, 16),
        'pkcs7'
      );
      const decoded = bytesToStr(decodedBytes, 'utf8');
  ...
  if (/window\.comicInfo/.test(value)) {
    try {
      ...
      if (match?.[1]) {
        // Node `new vm.Script('(obj)').runInNewContext()` → new Function 求值对象字面量。
        // eslint-disable-next-line no-new-func
        const comicInfo = new Function(`"use strict";return (${match[1]});`)();
...
function proxyChapterImages(content: string, source: BookSource) {
  ...
  // 原项目走服务端 /api/books/image 注入 Referer/UA 防盗链。DouyTV 无服务端,
  // 改走 dyproxy wrapImage(二进制透传 + CORS,注入 UA/Referer)。书源 header 里
  // 常带 Referer/User-Agent,漫画图床防盗链靠它,这里解析出来透传给 dyproxy。
  const header = asObjectHeader(rule?.header, { baseUrl: sourceBase(source), source: rule });
  const proxyHeaders: Record<string, string> = {};
  const referer = header['Referer'] || header['referer'] || sourceBase(source);
  ...
  return content.replace(/<img\b([^>]*?)\bsrc=(['"])(.*?)\2([^>]*)>/gi, (match, before, quote, rawSrc, after) => {
    if (!rawSrc || rawSrc.startsWith('dyproxy') || rawSrc.includes('dyproxy.localhost')) return match;
    const optionIndex = rawSrc.indexOf(',{');
    const src = optionIndex > 0 ? rawSrc.slice(0, optionIndex) : rawSrc;
    const absolute = normalizeUrl(sourceBase(source), src);
    const proxied = wrapImage(absolute, proxyHeaders) || absolute;
    return `<img${before}src=${quote}${proxied}${quote}${after}>`;
  });
}

// 原项目从 admin config + env(OPDS_ENABLED / LEGADO_SOURCES_JSON)拿书源;
// DouyTV 无服务端,书源全部来自 config.ts 读的 localStorage(手动书源 + 订阅展开)。
// enabled 恒为 true:只要 store 里有启用的 Legado 书源就算开启,由 UI 控制增删。
async function resolveLegadoConfig(): Promise<ResolvedLegadoConfig> {
  let sources: BookSource[] = [];
  try {
    sources = (await resolveLegadoSources()).map((source) => resolveLegadoSource(source));
  } catch {
    sources = [];
  }
  const filtered = sources.filter((source) => !!source.url && source.enabled !== false);
  return { enabled: filtered.length > 0, cacheTTL: LEGADO_CACHE_TTL_MS, sources: filtered };
}

/**
 * 把导入的 Legado 书源 JSON(单条或数组)规整成 BookSource[]。
 * 导出名对齐 DouyTV 侧调用点(BookSourcesHub 的 `normalizeImportedLegadoSources`)。
 */
export function normalizeImportedLegadoSources(input: unknown): BookSource[] {
...
async function fetchText(source: BookSource, url: string): Promise<string> {
  ...
  // Node 端 `validateProxyUrlServerSide`(SSRF 防护)在本地应用无服务端可保护,
  // 降级为基本的 http(s) 协议校验,拦截 file:// / javascript: 之类。
  if (!isHttpUrl(request.url)) throw new Error(`书源地址未通过安全校验: ${request.url}`);
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      // readingFetchText 走 Rust script_http_bytes:绕 CORS + 原始字节按 charset(gbk/gb18030)解码。
      // 显式 charset(书源 ,{charset:"gbk"})优先,否则 net 层按响应头/HTML meta 嗅探。
      const response = await readingFetchText(request.url, {
        method,
        headers,
      ...
      if (source.legado?.enabledCookieJar) mergeSetCookie(source.id, response.headers['set-cookie'] || null);
      if (!response.ok) throw new Error(`请求失败: ${response.status}`);
      const text = response.text;
      if (text.length > MAX_TEXT_BYTES) throw new Error('响应内容过大');
      textCache.set(cacheKey, { data: text, expiresAt: Date.now() + LEGADO_CACHE_TTL_MS });
      return text;
    } catch (error) {
      lastError = error;
...
function encodeExploreTarget(target: ExploreTarget) {
  return `legado-explore:${toBase64Url(JSON.stringify(target))}`;
}

function decodeExploreTarget(href?: string): ExploreTarget | null {
  if (!href?.startsWith('legado-explore:')) return null;
  try {
    const raw = fromBase64Url(href.slice('legado-explore:'.length));
    const parsed = JSON.parse(raw);
