...
import { decodeKrc } from './krcDecode'
...
export default {
  ...
  searchLyric(name, hash, time, tryNum = 0) {
    ...
    // console.log('[KG Lyric] DEBUG - Search URL:', url)
    ...
    requestObj.promise = requestObj.promise.then(({ body, statusCode }) => {
      // console.log('[KG Lyric] DEBUG - Search statusCode:', statusCode)
      // console.log('[KG Lyric] DEBUG - Search body:', JSON.stringify(body, null, 2))
      ...
      if (body.candidates && body.candidates.length) {
        ...
        // console.log('[KG Lyric] DEBUG - Found candidate:', info)
        return { id: info.id, accessKey: info.accesskey, fmt: (info.krctype == 1 && info.contenttype != 1) ? 'krc' : 'lrc' }
      }
      // console.log('[KG Lyric] DEBUG - No candidates found in response')
      return null
    })
    return requestObj
  },
