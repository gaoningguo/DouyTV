...
const SEARCH_PRESETS = ["赵雷", "周杰伦", "粤语", "民谣", "新歌", "热歌榜", "纯音乐", "影视原声"];
  ...
  const parse = (text: string) => {
    ...
    text.split("\n").forEach((line) => {
      ...
      matches.forEach((match) => {
        ...
        map.set(minutes * 60 + seconds + millis / 1000, content);
      });
    });
    return map;
  };
  const main = parse(lyricText || "");
  const trans = parse(tlyricText || "");
  return Array.from(new Set([...main.keys(), ...trans.keys()]))
    .sort((a, b) => a - b)
    .map((time) => ({
      time,
      text: main.get(time) || trans.get(time) || "",
      trans: trans.get(time) && trans.get(time) !== main.get(time) ? trans.get(time) : "",
    }))
...
export default function Music() {
  ...
  const playSong = useCallback(
    async (song: MusicSong, contextSongs: MusicSong[] = []) => {
      ...
      for (const candidate of candidates) {
        const source = sources.find((item) => item.enabled && item.id === candidate.sourceId);
        if (!source) continue;
      ...
      if (requestId === playRequestRef.current) {
        const message =
          lastError instanceof Error ? lastError.message : "获取播放地址失败";
        setError(message);
        pendingAutoPlayRef.current = false;
  ...
  const doSearch = useCallback(
    ...
    [activeSource, activeSourceId, enabledSources, keyword]
  );
          ...
          <header className="music-topbar shrink-0 px-4 sm:px-6">
            <div className="h-16 flex items-center gap-3">
              ...
              <nav className="ml-3 flex items-center gap-6 min-w-0 overflow-x-auto scrollbar-hide" aria-label="音乐导航">
                ...
                <TextTab active={view === "search"} onClick={() => setView("search")}>
                  搜索
                </TextTab>
              </nav>
...
function SearchView({
  ...
  const [category, setCategory] = useState<"all" | "songs" | "artists" | "albums">("all");
  ...
  return (
    <div className="music-ob-search space-y-10 pb-4">
      <section className="space-y-6">
        {trimmed ? (
          ...
          <h1 className="font-display text-2xl font-extrabold sm:text-3xl">搜索音乐</h1>
        )}
        <form onSubmit={submit} className="flex gap-2">
          <label className="search-field-shell music-search-field h-12 flex-1 min-w-0 flex items-center gap-2 px-3">
            <IconSearch size={18} className="text-cream-faint shrink-0" />
            <input
              value={keyword}
              onChange={(event) => onKeyword(event.target.value)}
            ...
            />
            {keyword && (
              <button type="button" onClick={() => onKeyword("")} className="text-cream-faint hover:text-cream">
                <IconClose size={15} />
              </button>
            )}
          </label>
          <button type="submit" className="music-search-submit" title="搜索">
            {searching ? <IconRefresh size={17} className="animate-spin" /> : <IconSearch size={17} />}
          </button>
        </form>
        {hasResults && (
      ...
      {!hasResults && !trimmed && (
        <section className="music-panel">
          <SectionHeader title="发现灵感" meta="快速搜索" />
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {SEARCH_PRESETS.map((item, index) => (
              <button
                key={item}
                type="button"
              ...
              >
                ...
                <span className="font-display text-sm font-semibold">{item}</span>
              </button>
            ))}
          </div>
        </section>
      ...
      {!searching && trimmed && !hasResults && (
        <EmptyBlock text="没有搜索结果" />
      )}
...
function LibraryView({
  ...
  return (
    <div className="music-library space-y-5 pb-4">
      <section className="music-profile">
        <div className="music-profile-avatar">
          {favoriteCover || recentCover ? (
            <img src={wrapImage(favoriteCover || recentCover)} alt="" className="h-full w-full rounded-full object-cover" />
          ) : (
            <IconAlbum size={38} />
          )}
        </div>
        <div className="min-w-0 text-center">
          <h1 className="font-display text-xl font-extrabold">我的音乐</h1>
          <p className="mt-1 text-xs text-cream-faint">收藏、历史和自建歌单</p>
        </div>
        <div className="grid w-full max-w-md grid-cols-3 gap-2">
          <StatCard label="收藏" value={favorites.length} />
          <StatCard label="历史" value={history.length} />
          <StatCard label="歌单" value={playlists.length} />
        </div>
      </section>

      <section className="music-favorite-card">
        <div className="relative z-10 flex min-w-0 items-center gap-4">
          <div className="grid h-20 w-20 shrink-0 place-items-center rounded-lg bg-ember-soft text-ember">
            <IconHeartFill size={34} />
          </div>
          <div className="min-w-0 flex-1">
            <h2 className="font-display text-xl font-bold">我喜欢的音乐</h2>
            <p className="mt-1 text-sm text-cream-faint">{favorites.length} 首歌曲</p>
          </div>
          <button
            type="button"
            disabled={favorites.length === 0}
            onClick={() => favorites[0] && onPlay(favorites[0], favorites)}
            className="music-floating-play disabled:opacity-40"
            title="播放喜欢的音乐"
          >
            <IconPlay size={20} />
          </button>
      ...
      <div className="music-library-tabs">
        <TextTab active={tab === "favorites"} onClick={() => onTab("favorites")}>收藏</TextTab>
        <TextTab active={tab === "history"} onClick={() => onTab("history")}>最近播放</TextTab>
        <TextTab active={tab === "playlists"} onClick={() => onTab("playlists")}>歌单</TextTab>
        {tab === "history" && history.length > 0 && (
          <button type="button" onClick={onClearHistory} className="ml-auto text-xs text-cream-faint hover:text-ember tap">
            清空
          </button>
        )}
        {tab === "playlists" && (
          <button type="button" onClick={onCreatePlaylist} className="ml-auto music-secondary-action">
            <IconPlus size={14} />
            新建
          </button>
        )}
      ...
      {tab === "playlists" ? (
        ...
        <section className="music-panel">
          <SongList
            songs={librarySongs}
            activeSong={currentSong}
            emptyText={tab === "favorites" ? "还没有收藏歌曲" : "还没有播放历史"}
            isFavorite={isFavorite}
            onPlay={(song) => onPlay(song, librarySongs)}
            onFavorite={onFavorite}
            onQueue={onQueue}
            onAddToPlaylist={onAddToPlaylist}
          />
...
function PlaylistPanel({
  ...
  return (
    <section className="rounded-lg overflow-hidden" style={{ background: "rgba(242,232,213,0.045)", border: "1px solid var(--cream-line)" }}>
      <div className="p-3 flex items-center gap-3">
        <CoverArt src={wrapImage(playlist.cover)} title={playlist.name} size="list" />
        <div className="min-w-0 flex-1">
          <h3 className="font-display font-semibold text-sm line-clamp-1">{playlist.name}</h3>
          <p className="text-xs text-cream-faint">{playlist.songs.length} 首</p>
        </div>
        {playlist.songs[0] && (
          <IconButton label="播放全部" onClick={() => onPlay(playlist.songs[0])}>
            <IconPlay size={15} />
          </IconButton>
        )}
        <IconButton label={open ? "收起" : "展开"} onClick={() => setOpen((value) => !value)}>
          <IconChevronRight size={15} style={{ transform: open ? "rotate(90deg)" : undefined }} />
        </IconButton>
        <IconButton label="清空" onClick={onClear}><IconTrash size={15} /></IconButton>
        <IconButton label="删除歌单" onClick={onDelete}><IconClose size={15} /></IconButton>
      </div>
      {open && (
            ...
            <div className="music-lyrics-stage">
              {lyricLines.length === 0 ? (
                ...
                lyricLines.map((line, index) => (
                  ...
                  >
                    <p className="font-display font-semibold">{line.text || line.trans}</p>
              ...
              <div ref={lyricRef} className="music-now-lyrics">
                {lyricLines.length === 0 ? (
                  ...
                  lyricLines.map((line, index) => (
                    ...
                    >
                      <span>{line.text || line.trans}</span>
...
function StatCard({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-lg p-3" style={{ background: "rgba(242,232,213,0.045)", border: "1px solid var(--cream-line)" }}>
